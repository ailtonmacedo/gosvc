...
func AddResource(request AddResourceRequest) (Result, bool, error) {
	...
	definition, err := preset.ResolveVersion(config.Project.Preset, config.Project.PresetVersion)
	if err != nil {
	...
	if err := Apply(request.ProjectDir, changes, artifacts, ApplyOptions{
		...
		Preset: definition.Name, PresetVersion: definition.Version, Features: definition.Features,
