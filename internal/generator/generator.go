...
func Generate(request Request) (Result, error) {
	...
	definition, err := preset.Resolve(request.Config.Project.Preset)
	if err != nil {
		return Result{}, err
	}
	...
	if len(resources) == 0 && (definition.Name == "postgres-api" || definition.Name == "production-api" || definition.Name == "event-driven-api") {
	...
	if existing != nil {
		...
		metadataChanged = existing.FrameworkVersion != request.FrameworkVersion ||
			existing.Preset != definition.Name || !sameCoreFeatures(existing.Features, definition.Features) ||
