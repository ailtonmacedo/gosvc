...
func (m Metadata) Validate(gosvcVersion string) error {
	...
	if m.SchemaVersion != 1 && m.SchemaVersion != CurrentSchemaVersion {
		issues = append(issues, fmt.Sprintf("schema_version must be 1 or %d", CurrentSchemaVersion))
	}
	...
	if !m.BuiltIn && m.SchemaVersion == CurrentSchemaVersion {
		...
		if strings.TrimSpace(m.Entrypoint) == "" {
			issues = append(issues, "entrypoint is required for executable plugins")
		}
		if !checksumPattern.MatchString(m.Checksum) {
			issues = append(issues, "checksum must use sha256:<64 lowercase hexadecimal characters>")
		}
	}
	if m.SchemaVersion == 1 && (m.ProtocolVersion != 0 || m.Checksum != "") {
...
func (m Metadata) ExecutionReady() error {
	...
	if m.SchemaVersion != CurrentSchemaVersion {
		return fmt.Errorf("plugin %q uses legacy schema %d; upgrade plugin.json to schema %d before execution", m.Name, m.SchemaVersion, CurrentSchemaVersion)
	}
	if m.ProtocolVersion != CurrentProtocolVersion {
