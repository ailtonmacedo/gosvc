...
Commands:
  ...
  doctor           Check development tools for a preset or project
  ...
  presets          List and inspect versioned built-in presets
...
func (a *App) runVersion(args []string) error {
	...
	fmt.Fprintf(a.stdout, "gosvc version %s\ncommit: %s\nbuilt: %s\nexecutable: %s\n",
		buildinfo.Version, buildinfo.Commit, buildinfo.BuildTime, executable)
	return nil
}
...
Options:
  --preset <name>          Preset: bare, worker, minimal-api, postgres-api, production-api, or event-driven-api
  --preset-version <ver>   Exact preset version; defaults to the current version for that preset
  --router <name>          HTTP router: chi (default) or echo (minimal-api/postgres-api)
  --persistence <name>     PostgreSQL persistence: sqlc (default) or gorm (postgres-api only)
  --output <path>          Destination directory; defaults to a sibling of the gosvc source repo
                           when invoked from inside gosvc, otherwise ./<project-name>
...
Flags may appear before or after the project name.`)
	...
	definition, err := preset.ResolveVersion(options.Preset, options.PresetVersion)
	if err != nil {
		return clierror.Wrap(clierror.CodeInvalidConfig, "resolve preset", err)
	}
	...
	if options.Router != "" {
		config.API.Router = options.Router
		if options.Router == "echo" && config.GoLanguageVersion() == "auto" {
	...
	if options.Persistence != "" {
		...
		case "sqlc":
			config.Database.Driver = "pgx"
			config.Database.Pool = "pgxpool"
			config.Database.CodeGeneration = "sqlc"
		case "gorm":
			config.Database.Driver = "gorm-postgres"
			config.Database.Pool = "database/sql"
			config.Database.CodeGeneration = "gorm"
		default:
			return clierror.New(clierror.CodeInvalidConfig, "--persistence must be sqlc or gorm")
		}
	}
	...
	destination, err := resolveNewDestination(options.Name, options.Destination)
	if err != nil {
		return clierror.Wrap(clierror.CodeInvalidConfig, "resolve project destination", err)
	}
	if strings.TrimSpace(options.Destination) == "" {
		if legacy, ok := legacyNestedDestination(options.Name, destination); ok {
			fmt.Fprintf(a.stderr, "warning: legacy project directory %s still exists inside gosvc; the new project is generated at %s. Do not cd into the legacy directory.\n", legacy, destination)
		}
	}
	...
	fmt.Fprintf(a.stdout, "Project generated at %s using preset %s@%s.\n", result.Destination, result.Preset.Name, result.Preset.Version)
	fmt.Fprintf(a.stdout, "Next: cd %s\n", result.Destination)
	fmt.Fprintln(a.stdout, "Then: make bootstrap")
	return nil
}

func (a *App) runPresets(args []string) error {
	if len(args) == 0 || args[0] == "list" {
		if len(args) > 1 {
			return clierror.New(clierror.CodeInvalidConfig, "usage: gosvc presets list")
		}
		for _, definition := range preset.Definitions() {
			fmt.Fprintf(a.stdout, "%-18s %-8s %-7s %s\n", definition.Name, definition.Version, definition.Kind, definition.Description)
		}
		return nil
	}
	if args[0] != "show" || (len(args) != 2 && len(args) != 3) {
		return clierror.New(clierror.CodeInvalidConfig, "usage: gosvc presets <list|show> [name] [version]")
	}
	version := ""
	if len(args) == 3 {
		version = args[2]
	}
	definition, err := preset.ResolveVersion(args[1], version)
	if err != nil {
		return clierror.Wrap(clierror.CodeInvalidConfig, "resolve preset", err)
	}
	fmt.Fprintf(a.stdout, "Preset: %s\nVersion: %s\nKind: %s\nDescription: %s\n", definition.Name, definition.Version, definition.Kind, definition.Description)
	if len(definition.Components.Routers) > 0 {
		fmt.Fprintf(a.stdout, "Routers: %s (default: %s)\n", strings.Join(definition.Components.Routers, ", "), definition.Components.DefaultRouter)
	}
	if len(definition.Components.Persistence) > 0 {
		fmt.Fprintf(a.stdout, "Persistence: %s (default: %s)\n", strings.Join(definition.Components.Persistence, ", "), definition.Components.DefaultPersistence)
	}
	fmt.Fprintln(a.stdout, "Features:")
...
func (a *App) runDoctor(args []string) error {
	...
	flags.SetOutput(io.Discard)
	projectDir := flags.String("project", ".", "gosvc project directory")
	presetName := flags.String("preset", "", "check prerequisites for a preset before generation")
	if err := flags.Parse(args); err != nil {
		return clierror.Wrap(clierror.CodeInvalidConfig, "invalid doctor arguments", err)
	}
	if flags.NArg() != 0 {
		return clierror.New(clierror.CodeInvalidConfig, "usage: gosvc doctor [--project <path>] [--preset <name>]")
	}
	if *presetName != "" {
		if _, err := preset.Resolve(*presetName); err != nil {
			return clierror.Wrap(clierror.CodeInvalidConfig, "invalid doctor preset", err)
		}
	}
	report, err := doctor.CheckForPreset(*projectDir, *presetName)
	if err != nil {
	...
	for _, result := range report.Results {
		...
		if !result.Tool.Required && result.Tool.Local && result.Status != doctor.StatusOK {
			if details != "" {
				details += " — optional before generation; make bootstrap installs it"
			} else {
				details = "optional before generation; make bootstrap installs it"
			}
		}
...
func (a *App) runUpgrade(args []string) error {
	...
	targetPresetVersion := flags.String("preset-version", "", "target version of the project's existing preset")
	...
	if flags.NArg() != 0 {
		return clierror.New(clierror.CodeInvalidConfig, "usage: gosvc upgrade [--project <path>] [--to <version>] [--preset-version <version>] [--dry-run] [--force] [--no-backup]")
	}
	result, err := upgrade.Run(upgrade.Options{
		ProjectDir: *projectDir, TargetVersion: *target, TargetPresetVersion: *targetPresetVersion,
	...
	fmt.Fprintf(a.stdout, "Preset: %s -> %s\n", displayVersion(result.FromPresetVersion), displayVersion(result.ToPresetVersion))
...
func (a *App) runPlugins(args []string) error {
	...
	for _, item := range plugins {
		...
		if item.BuiltIn {
			...
			if err := item.ExecutionReady(); err != nil {
				return clierror.Wrap(clierror.CodeInvalidProject, "validate plugin "+item.Name, err)
			}
			if item.IsSandboxed() {
				if _, lookErr := exec.LookPath("docker"); lookErr != nil {
					return clierror.Wrap(clierror.CodeInvalidProject, "validate docker plugin "+item.Name, lookErr)
				}
				status = "sandboxed"
			} else {
		...
		mode := "builtin"
		if !item.BuiltIn {
			mode = string(item.ExecutionMode())
		}
		fmt.Fprintf(a.stdout, "%-16s %-10s %-10s %-10s %-12s %s\n", item.Name, item.Version, mode, status, source, item.Description)
	}
	if action == "validate" {
...
func (a *App) runExternalPlugin(args []string) error {
	...
	result, err := pluginapply.Run(context.Background(), pluginapply.Options{
		...
		RequireSandbox: options.requireSandbox, AllowNetwork: options.allowNetwork,
	})
	if err != nil {
		code := clierror.CodeGenerationFailure
		if generator.IsConflict(err) {
...
func parsePluginRunOptions(args []string) (pluginRunOptions, error) {
	...
	if options.name == "" {
		return pluginRunOptions{}, fmt.Errorf("usage: gosvc plugins run <name> [--project <path>] [--dry-run] [--force] [--timeout 10s] [--require-sandbox] [--allow-network]")
	}
	return options, nil
}
