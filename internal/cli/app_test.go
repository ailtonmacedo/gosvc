...
func TestRunAcceptanceJSON(t *testing.T) {
	...
	if report.SchemaVersion != 1 || report.Passed != 4 || report.Failed != 0 || len(report.Presets) != 4 {
...
func TestRunCertifyStatic(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "Certification: passed=4 failed=0 blocked=0") {
		t.Fatalf("stdout=%q", stdout.String())
	}
}
