...
func certifyPreset(root, name string, options Options) (result PresetResult) {
	...
	blockers := realBlockers(name, config.Project.GoVersion, activeGoVersion(nil))
	if len(blockers) > 0 {
	...
	if name == "minimal-api" {
		...
		if name == "event-driven-api" {
			...
			services = append(services, "redis", "kafka")
		}
...
func realBlockers(presetName, requiredGo, actualGo string) []string {
	var blockers []string
	...
	required := []string{"go", "git", "docker", "golangci-lint", "govulncheck"}
	if presetName != "minimal-api" {
		required = append(required, "sqlc", "oapi-codegen", "migrate")
	}
...
func goVersionAtLeast(actualRuntime, required string) bool {
	actual := strings.TrimPrefix(actualRuntime, "go")
	if strings.Contains(actual, "devel") {
		return true
	}
	aMajor, aMinor := majorMinor(actual)
	rMajor, rMinor := majorMinor(required)
	if aMajor != rMajor {
...
func majorMinor(value string) (int, int) {
...
func smokeAPI(result *PresetResult, timeout time.Duration, dir, presetName string, baseEnv []string, cfg project.Config) error {
	...
	if presetName == "production-api" || presetName == "event-driven-api" {
		...
		checks = append(checks, "login", "jwt", "rbac", "refresh", "logout", "metrics", "orders-crud")
	}
