...
func TestRunMigratesLegacyManifestAndPreservesUserFiles(t *testing.T) {
	...
	if !result.Applied || result.FromSchemaVersion != 1 || result.ToSchemaVersion != 4 || result.BackupPath == "" {
...
func TestRunUpgradesPresetVersionExplicitly(t *testing.T) {
	t.Parallel()
	dir := filepath.Join(t.TempDir(), "service")
	config := project.DefaultConfigForPreset("postgres-api")
	config.Project.Name = "service"
	config.Project.Module = "github.com/acme/service"
	config.Project.PresetVersion = "1.0.0"
	if _, err := generator.Generate(generator.Request{Config: config, Destination: dir, FrameworkVersion: "1.1.0"}); err != nil {
		t.Fatal(err)
	}
	result, err := Run(Options{ProjectDir: dir, TargetVersion: "1.1.0", TargetPresetVersion: "1.1.0", RuntimeVersion: "dev", NoBackup: true})
	if err != nil {
		t.Fatal(err)
	}
	if !result.Applied || result.FromPresetVersion != "1.0.0" || result.ToPresetVersion != "1.1.0" {
		t.Fatalf("result=%+v", result)
	}
	loaded, err := project.Load(filepath.Join(dir, "project.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if loaded.Project.PresetVersion != "1.1.0" {
		t.Fatalf("project preset version=%q", loaded.Project.PresetVersion)
	}
	upgraded, err := manifest.Load(dir)
	if err != nil {
		t.Fatal(err)
	}
	if upgraded.PresetVersion != "1.1.0" {
		t.Fatalf("manifest preset version=%q", upgraded.PresetVersion)
	}
}
