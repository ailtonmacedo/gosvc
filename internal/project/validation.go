...
func (c Config) Validate() error {
	...
	var definition presetpkg.Definition
	if c.Project.Preset == "" {
		add("project.preset", nil, "is required")
	} else {
		resolved, err := presetpkg.ResolveVersion(c.Project.Preset, c.Project.PresetVersion)
		if err != nil {
			add("project.preset_version", c.Project.PresetVersion, err.Error())
		} else {
			definition = resolved
		}
	}
	language := normalizeGoVersion(c.GoLanguageVersion())
	if language == "" {
		add("runtime.go.language", nil, "is required")
	} else if language != "auto" {
		if _, _, _, _, ok := parseGoVersion(language); !ok {
			add("runtime.go.language", c.GoLanguageVersion(), "must be auto or a Go version such as 1.25.0")
		}
	}
	toolchain := strings.TrimSpace(c.GoToolchainVersion())
	if toolchain != "" {
		if !strings.HasPrefix(toolchain, "go") {
			add("runtime.go.toolchain", toolchain, "must use Go toolchain syntax such as go1.25.12")
		} else if _, _, _, _, ok := parseGoVersion(normalizeGoVersion(toolchain)); !ok {
			add("runtime.go.toolchain", toolchain, "must use Go toolchain syntax such as go1.25.12")
		} else if language != "auto" && compareGoVersions(normalizeGoVersion(toolchain), BuildGoVersion(language)) < 0 {
			add("runtime.go.toolchain", toolchain, fmt.Sprintf("must be at least go%s for language/security compatibility", BuildGoVersion(language)))
		}
	}
	...
	if c.API.Enabled {
		if definition.Name != "" && !containsString(definition.Components.Routers, c.API.Router) {
			add("api.router", c.API.Router, fmt.Sprintf("unsupported router for %s@%s; allowed values: %s", definition.Name, definition.Version, strings.Join(definition.Components.Routers, ", ")))
		}
	...
	if c.Database.Enabled {
		if definition.Name != "" && !containsString(definition.Components.Persistence, c.Database.CodeGeneration) {
			add("database.code_generation", c.Database.CodeGeneration, fmt.Sprintf("unsupported persistence for %s@%s; allowed values: %s", definition.Name, definition.Version, strings.Join(definition.Components.Persistence, ", ")))
		}
		...
		switch c.Database.CodeGeneration {
		case "sqlc":
			if c.Database.Driver != "pgx" {
				add("database.driver", c.Database.Driver, "sqlc persistence requires driver pgx")
			}
			if c.Database.Pool != "pgxpool" {
				add("database.pool", c.Database.Pool, "sqlc persistence requires pool pgxpool")
			}
		case "gorm":
			if c.Database.Driver != "gorm-postgres" {
				add("database.driver", c.Database.Driver, "gorm persistence requires driver gorm-postgres")
			}
			if c.Database.Pool != "database/sql" {
				add("database.pool", c.Database.Pool, "gorm persistence requires pool database/sql")
			}
		default:
			add("database.code_generation", c.Database.CodeGeneration, "unsupported persistence; allowed values: sqlc, gorm")
		}
	}
	...
	if c.Project.Preset == "minimal-api" || c.Project.Preset == "postgres-api" || c.Project.Preset == "production-api" || c.Project.Preset == "event-driven-api" {
		if !c.API.Enabled {
			add("api.enabled", c.API.Enabled, "must be true for API presets")
		}
	}
	if c.Project.Preset == "bare" || c.Project.Preset == "worker" {
		if c.API.Enabled {
			add("api.enabled", c.API.Enabled, "must be false for non-HTTP presets")
		}
		if c.OpenAPI.Enabled {
			add("openapi.enabled", c.OpenAPI.Enabled, "must be false for non-HTTP presets")
		}
		if c.Database.Enabled {
			add("database.enabled", c.Database.Enabled, "must be false for bare and worker presets")
		}
	}
	...
	if c.Project.Preset == "production-api" || c.Project.Preset == "event-driven-api" {
		if c.API.Router != "chi" {
			add("api.router", c.API.Router, "production-api and event-driven-api currently require chi")
		}
		if c.Database.CodeGeneration != "sqlc" {
			add("database.code_generation", c.Database.CodeGeneration, "production-api and event-driven-api currently require sqlc")
		}
		if !c.Auth.Enabled {
