...
func decodeProjectConfig(root map[string]any) (Config, error) {
	if err := rejectUnknown(root, "", "schema_version", "project", "runtime", "architecture", "api", "openapi", "database", "auth", "rate_limit", "cache", "messaging", "outbox", "observability", "performance", "deployment", "quality"); err != nil {
		return Config{}, err
	}
	...
	legacyGoVersion := ""
	legacyGoVersionSet := false
	if section, exists, err := optionalMap(root, "project"); err != nil {
		return Config{}, err
	} else if exists {
		if err := rejectUnknown(section, "project", "name", "module", "go_version", "preset", "preset_version"); err != nil {
			return Config{}, err
		}
		...
		if _, ok := section["go_version"]; ok {
			legacyGoVersionSet = true
			legacyGoVersion, err = optionalString(section, "go_version", config.Project.GoVersion, "project.go_version")
			if err != nil {
				return Config{}, err
			}
			config.Project.GoVersion = legacyGoVersion
			config.Runtime.Go.Language = legacyGoVersion
			config.Runtime.Go.Toolchain = PreferredToolchain(legacyGoVersion)
		}
		config.Project.Preset, err = optionalString(section, "preset", config.Project.Preset, "project.preset")
		if err != nil {
			return Config{}, err
		}
		config.Project.PresetVersion, err = optionalString(section, "preset_version", config.Project.PresetVersion, "project.preset_version")
		if err != nil {
			return Config{}, err
		}
	}

	if section, exists, err := optionalMap(root, "runtime"); err != nil {
		return Config{}, err
	} else if exists {
		if err := rejectUnknown(section, "runtime", "go"); err != nil {
			return Config{}, err
		}
		goSection, goExists, mapErr := optionalMap(section, "go")
		if mapErr != nil {
			return Config{}, mapErr
		}
		if goExists {
			if err := rejectUnknown(goSection, "runtime.go", "language", "toolchain"); err != nil {
				return Config{}, err
			}
			language, parseErr := optionalString(goSection, "language", config.Runtime.Go.Language, "runtime.go.language")
			if parseErr != nil {
				return Config{}, parseErr
			}
			toolchain, parseErr := optionalString(goSection, "toolchain", config.Runtime.Go.Toolchain, "runtime.go.toolchain")
			if parseErr != nil {
				return Config{}, parseErr
			}
			if legacyGoVersionSet && language != legacyGoVersion {
				return Config{}, fmt.Errorf("project.go_version=%q conflicts with runtime.go.language=%q; remove the legacy project.go_version field", legacyGoVersion, language)
			}
			config.Runtime.Go.Language = language
			config.Runtime.Go.Toolchain = toolchain
			config.Project.GoVersion = language
		}
	}
