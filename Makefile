...
.PHONY: build install test test-race vet fmt tidy verify run clean completions acceptance certify certify-real benchmark release-prepare release-check release-snapshot release-verify
...
install:
	mkdir -p $(INSTALL_DIR)
	go build -trimpath -ldflags="$(LDFLAGS)" -o $(INSTALL_DIR)/$(APP_NAME) ./cmd/gosvc
